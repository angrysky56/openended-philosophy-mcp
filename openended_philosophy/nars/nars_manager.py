...
class NARSManager:
    ...
    async def _send_command(self, command: str) -> None:
        ...
        assert self.process is not None, "NARS process should be started."
        assert self.process.stdin is not None, "NARS process stdin should be available."
    ...
    async def _get_output(self) -> list[str]:
        ...
        while True:
            if self.process is None:
                logger.warning("NARS process is None in _get_output loop; likely terminated concurrently.")
                break
            if self.process.stdout is None:
                # This case is less likely if Popen was initialized correctly with stdout=PIPE,
                # but check defensively.
                logger.error("NARS process.stdout is None in _get_output loop. This may indicate an issue with Popen setup or process state.")
                break

            line = await asyncio.to_thread(self.process.stdout.readline)
            if not line or "done with 0 additional inference steps." in line: # readline() returns empty string on EOF
                break
