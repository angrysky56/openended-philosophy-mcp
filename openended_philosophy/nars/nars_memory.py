...
class NARSMemory:
    ...
    def __init__(self, 
        ...
        self.embedding_model = embedding_model or TfidfVectorizer(max_features=1000)
    ...
    def query(self, term: str, occurrence_time: str = "eternal") -> Optional[MemoryItem]:
        """
    ...
    def get_attention_buffer(self, 
                            ...
                            include_categories: Optional[List[str]] = None) -> List[MemoryItem]:
        """
    ...
    def eternalize_beliefs(self) -> None:
        ...
        for (term, time), item in list(self.memory.items()):
            if time != "eternal" and isinstance(time, (int, float)):
                if self.current_time - float(time) > self.eternalization_distance:
    ...
    def _generate_embedding(self, text: str) -> np.ndarray:
        ...
        try:
            ...
            embedding = self.embedding_model.transform([text]).toarray()[0]
            return embedding
            
        except Exception as e:
            logger.warning(f"Failed to generate embedding: {e}")
            # Return random embedding as fallback
            return np.random.rand(self.embedding_model.max_features if hasattr(self.embedding_model, 'max_features') else 100)
